...
bool UnifiedLEDCController::runStartupDiagnostic(float maxPowerPercent, uint32_t durationMs) {
    if (!_initialized) {
        ESP_LOGE("MOTOR_LEDC", "❌ runStartupDiagnostic: контроллер не инициализирован!");
        return false;
    }

    // 🔒 Аппаратное ограничение безопасности
    maxPowerPercent = constrain(maxPowerPercent, 0.0f, 35.0f);
    ESP_LOGI("MOTOR_LEDC", "⚡ ЗАПУСК ПРЕДСТАРТОВОГО ТЕСТА МОТОРОВ (MAX: %.1f%%)", maxPowerPercent);

    auto runSingleMotorRamp = [this](uint8_t idx, float limit) {
        ESP_LOGI("MOTOR_LEDC", "  Мотор #%d: разгон 0%% → %.1f%%", idx, limit);
        for (int p = 0; p <= (int)limit; p += 5) { setMotorPower(idx, p); delay(50); }
        for (int p = (int)limit; p >= 0; p -= 5) { setMotorPower(idx, p); delay(50); }
    };

    runSingleMotorRamp(0, maxPowerPercent);
    delay(200);
    runSingleMotorRamp(1, maxPowerPercent);
    delay(200);

    // Совместная работа
    ESP_LOGI("MOTOR_LEDC", "  Оба мотора: %.1f%% (%d мс)", maxPowerPercent, durationMs/2);
    setMotorPower(0, maxPowerPercent);
    setMotorPower(1, maxPowerPercent);
    delay(durationMs / 2);

    stopAllMotors();
    ESP_LOGI("MOTOR_LEDC", "✅ Тест моторов завершён. Состояние: STOP");
    return true;
}
