...
:  _module(nullptr),                   // 2. Module* _module
  _radio(nullptr),                    // 3. SX1278* _radio
  _spi(spi),                          // 1. SPIClass* _spi
  // _instance(nullptr),                 // 4. static FastLoRaReceiver* _instance
  _currentState(StateRx::INIT),       // 5. StateRx _currentState
  _initialized(false),                // 6. bool _initialized
  _chipConnected(false),              // 7. bool _chipConnected
  _dataReady(false),                  // 8. volatile bool _dataReady
  _dataReceivedFlag(false),           // 9. volatile bool _dataReceivedFlag
  _dio1TimeoutFlag(false),            // 10. volatile bool _dio1TimeoutFlag
  _dio1CadFlag(false),                // 11. volatile bool _dio1CadFlag
  _receivedData{},                    // 12. DataComSet_t _receivedData
  _lastReceiveTime(0),                // 13. uint32_t _lastReceiveTime
  _errorCount(0),                     // 14. uint32_t _errorCount
  _lastErrorTime(0),                  // 15. uint32_t _lastErrorTime
  _packetStats{},                     // 16. ReceiverStats _packetStats
  _ackPacketId(0),                    // 17. volatile uint16_t _ackPacketId
  _lastData{},                        // 18. DataComSet_t _lastData
  _ackPacket{},                       // 19. AckPacket_t _ackPacket
  _lastPacketId(0),                   // 20. uint16_t _lastPacketId
  _config{},                          // 21. LoRaConfig_t _config
  _debugLevel(DebugLevel::DEBUG_INFO),// 22. DebugLevel _debugLevel
  _statsRx{}                          // 23. mutable ReceiverStats _statsRx
{
...
bool FastLoRaReceiver::diagnoseSPI() {
    ...
    ESP_LOGI(TAG_LORA_RX, "  • Пины настроены: CS=%d, RST=%d, DIO0=%d, MISO=%d",
             Config::Pins::LORA_CS, Config::Pins::LORA_RST, 
             Config::Pins::LORA_DIO0, Config::Pins::FSPI_MISO);
...
bool FastLoRaReceiver::begin() {
    // 🔍 1. Проверка указателей: предотвращаем утечку памяти при повторном вызове
    //            if (_module != nullptr) {
    //                ESP_LOGW(TAG_LORA_RX, "⚠️ begin(): Module уже существует. Освобождение памяти перед реинициализацией...");
    //                _radio->standby();
    //                delete _radio;
    //                delete _module;
    //                _radio = nullptr;
    ...
    if (Config::Pins::LORA_DIO1 != RADIOLIB_NC) {
        ...
        //         dio1_state ? "HIGH" : "LOW");
        ESP_LOGD(TAG_LORA_RX, "📊 DIO1 (GPIO%d) начальное состояние: %s", 
                Config::Pins::LORA_DIO1, dio1_state ? "HIGH" : "LOW");
    } else {
...
void FastLoRaReceiver::printFullReport() {
    ...
    ESP_LOGI(TAG_LORA_RX, "  Всего принято: %lu", stats.packetsReceived); // ← Замена
...
bool FastLoRaReceiver::diagnoseDIO1() {
    ...
    // ✅ ИСПОЛЬЗОВАТЬ pin_state:
    //ESP_LOGI(TAG_LORA_RX, "   📊 Состояние пина: %s (ожидаемо HIGH без активности)",
    //         pin_state ? "HIGH ✅" : "LOW ⚠️");
    ESP_LOGI(TAG_LORA_RX, "📊 DIO1 (GPIO%d): уровень=%s (ожидаемо HIGH)", 
         Config::Pins::LORA_DIO1, pin_state ? "HIGH ✓" : "LOW ⚠️");
