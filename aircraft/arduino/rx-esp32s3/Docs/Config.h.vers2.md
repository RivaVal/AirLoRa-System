...
//==================================================================
//  Также учтем, что для  моторов мы используем LEDC, 
//  поэтому настроим каналы соответственно.
//  ## 🔧 2. Обновлённый Config.h (настройки АКБ и ModuleConfig)

        /*
        //-------------------------------------------
        📊 Сводная таблица подключений:
        Модуль	    Интерфейс	Пины	Примечания
        Ra-01 SX1278	SPI	    (VSPI) CS=5, RST=13, DIO0=4	Промышленный стандарт
        ATGM336H GPS	UART	RX=16, TX=17	Высокая точность
        GY-87 IMU	    I2C	    SDA=21, SCL=22	10DOF сенсор
        ESP32-CAM	    I2C	    SDA=21, SCL=22	Общий I2C bus
        Сервоприводы	PWM	    25,26,27,32,15	5 сервоприводов / УСТАРЕЛО /
        Сервоприводы	I2C     SDA=21, SCL=22  PCA9685	 7-16 сервоприводов
        Моторы	        PWM	    32,14	2 мотора / УСТАРЕЛО /
        Моторы	        LEDC	32,33	2 мотора
        ...
        #define LORA_DIO0    4
//============================================================
Необходимо объяснить назначение выводов SPI интерфейса 
для LoRa модуля и как они подключаются к ESP32.

По таблице пользователя для LoRa используются пины:
 CS=5, RST=13, DIO0=4. Также в коде определены:
 LORA_SCK=18, LORA_MISO=19, LORA_MOSI=23, LORA_SS=5, LORA_RST=13, LORA_DIO0=4.

Выводы LoRa модуля (например, Ra-01 SX1278) обычно имеют следующие назначения:
    SCK: Serial Clock - тактовый сигнал SPI
    MISO: Master In Slave Out - данные от LoRa (ведомого) к ESP32 (ведущему)
    MOSI: Master Out Slave In - данные от ESP32 (ведущего) к LoRa (ведомому)
    NSS (или SS): Slave Select (также может называться CS - Chip Select) - выбор ведомого устройства
    RST: Reset - сброс модуля
    DIO0: Digital I/O 0 - используется для прерываний (например, при приеме пакета)

Подключение:
    SCK на LoRa подключается к LORA_SCK (GPIO18) на ESP32
    MISO на LoRa подключается к LORA_MISO (GPIO19) на ESP32
    MOSI на LoRa подключается к LORA_MOSI (GPIO23) на ESP32
    NSS (или CS) на LoRa подключается к LORA_SS (GPIO5) на ESP32
    RST на LoRa подключается к LORA_RST (GPIO13) на ESP32
    DIO0 на LoRa подключается к LORA_DIO0 (GPIO4) на ESP32
Теперь объясним, что такое SCK и SS (NSS):
    SCK (Serial Clock): это тактовый сигнал, который генерируется 
ведущим устройством (ESP32) для синхронизации передачи данных по SPI.
    SS (Slave Select) или CS (Chip Select): этот сигнал используется 
для выбора ведомого устройства (LoRa модуля). Когда на этом выводе 
низкий уровень, LoRa модуль активен и обменивается данными. Когда 
высокий уровень, модуль игнорирует сигналы на SPI шине.
    Пользователь в таблице указал для LoRa пины: CS=5, RST=13, DIO0=4, 
что соответствует коду. Также в коде указаны пины для SCK, MISO, MOSI (18,19,23). 
Значит, подключение должно быть следующим:
...
Теперь нужно нарисовать схему подключения в EasyEDA, 
но если пользователь не может разобраться с созданием 
символов, можно временно использовать стандартные компоненты 
и подписать пины вручную.


Отличное решение! Вернемся к практической задаче. Разберем подключение LoRa модуля RA-01 SX1278 - это действительно важный момент. 📡
...
#define LORA_SCK     18  // SCK на LoRa
#define LORA_MISO    19  // MISO на LoRa  
#define LORA_MOSI    23  // MOSI на LoRa
#define LORA_SS      5   // NSS/SS на LoRa
#define LORA_RST     13  // RST на LoRa
#define LORA_DIO0    4   // DIO0 на LoRa
...
// Настройки I2C EDP32-CAM
#define I2C_SDA 1
#define I2C_SCL 3
#define I2C_ADDRESS 0x08

🔍 Проверка подключения LoRa:
    Питание: LoRA модуль должен получать 3.3V и GND
    SPI: SCK, MISO, MOSI, SS - обязательные для работы
    Управление: RST и DIO0 - для контроля и прерываний
💡 Важно для SPI:
    SS (Slave Select) - активирует модуль когда на низком уровне (LOW)
    SCK (Serial Clock) - тактирование данных
    MISO/MOSI - фактическая передача данных
Ваша распиновка абсолютно корректна! Теперь давайте визуализируем это 
в схеме. Хотите, чтобы я помог нарисовать эту схему подключения в 
более простом инструменте? Или продолжим с EasyEDA? 🛠️
        */
...
// MOTORS: через LEDC напрямую к ESP32
//   Мотор #0 → GPIO 32
//   Мотор #1 → GPIO 33
...
// Файл: Config.h | Позиция: внутри namespace Config::Pins
//=============================================================================
    ...
    constexpr uint8_t LORA_CS    = 10;    // Chip Select - отдельный от SPI шины
    constexpr uint8_t LORA_RST   = 9;    // 🔑 ИЗМЕНЕНО: 13→9 (устранён конфликт с MISO)
    constexpr uint8_t LORA_DIO0  = 4;     // Прерывание пакета (RxDone)
    constexpr uint8_t LORA_DIO1  = 14;    // Прерывание таймаута (RxTimeout / CAD (RadioLib 7.6.0+)

    // 🔌 FSPI шина (стандарт для ESP32-S3)
    // 🔑 ВАЖНО: Пины FSPI не должны пересекаться с LORA_RST/LORA_CS
    constexpr uint8_t FSPI_SCLK  = 12;    // SPI Clock
    constexpr uint8_t FSPI_MISO  = 13;    // MISO ← ОСТАВЛЯЕМ 13
    constexpr uint8_t FSPI_MOSI  = 11;    // MOSI

    // 🔗 I2C шина (общая для MPU9250 + PCA9685 + другие)
    constexpr uint8_t I2C_SDA    = 8;     // I2C Data - ADC1_CH7, безопасен
    constexpr uint8_t I2C_SCL    = 9;     // 🔑 ИЗМЕНЕНО: 33→9 (I2C на ADC1 пинах для стабильности)
    ...
    // 🔑 GPIO7 = ADC1_CH6: поддерживает калибровку, не конфликтует с USB
    // constexpr uint8_t BATTERY_ADC_PIN = 7;
    constexpr uint8_t BATTERY_ADC_PIN = 7;   // 🔑 РЕКОМЕНДУЕМЫЙ ПИН ДЛЯ S3!
    ...
    // 🔔 Периферия
    constexpr uint8_t BUZZER_PIN = 16;    // 🔑 ИЗМЕНЕНО: 12→16 (устранён конфликт с SCLK)
    constexpr uint8_t LED_STATUS = 2;     // Индикатор состояния (безопасный пин)

              //          // 🔍 Диагностический вывод конфигурации
              //          ESP_LOGI("CONFIG", "✅ ESP32-S3 Pin Map validated:");
              //          ESP_LOGI("CONFIG", "   LoRa: CS=%d, RST=%d, DIO0=%d, DIO1=%d", 
              //                  LORA_CS, LORA_RST, LORA_DIO0, LORA_DIO1);
              //          ESP_LOGI("CONFIG", "   FSPI: SCLK=%d, MISO=%d, MOSI=%d", 
              //                  FSPI_SCLK, FSPI_MISO, FSPI_MOSI);
              //          ESP_LOGI("CONFIG", "   I2C: SDA=%d, SCL=%d", I2C_SDA, I2C_SCL);
              //          ESP_LOGI("CONFIG", "   Motors: GPIO%d, GPIO%d", motorPins[0], motorPins[1]);
              //          ESP_LOGI("CONFIG", "   Battery ADC: GPIO%d (ADC1_CH6)", BATTERY_ADC_PIN);
              //          // #endif
#else
    ...
    namespace Timing {   
        ...
        //                    // Настройки I2C
        //                    constexpr uint32_t I2C_FREQUENCY = 400000; // Fast mode 400kHz
    ...
    namespace LEDCConfig {
        ...
        constexpr uint32_t CHANNEL_SETUP_DELAY_MS = 10; // Задержка между настройкой каналов
            ...
            //                // Тайминги сервоприводов
            //                constexpr uint32_t SERVO_FREQ = 50;           // Hz
            //                constexpr uint32_t SERVO_RESOLUTION = 16;     // бит
            //                constexpr uint32_t SERVO_MIN_PULSE = 544 ;    // 500;     // мкс
            //                constexpr uint32_t SERVO_MAX_PULSE = 2400 ;   // 2500;    // мкс
            //                constexpr uint32_t SERVO_NEUTRAL = 1500;      // мкс
...
namespace ValidationLimits {
    ...
    constexpr uint8_t MOTOR_COUNT = 2;
            ...
            //                    constexpr int SERVO_MIN_ANGLE = 0;      // Физический минимум (0°)
            //                    constexpr int SERVO_MAX_ANGLE = 180;    // Физический максимум (180°)
            //                    constexpr int SERVO_SAFE_MIN_ANGLE = 5;       // Безопасный минимум с запасом
            //                    constexpr int SERVO_SAFE_MAX_ANGLE = 175;     // Безопасный максимум с запасом
            //                    constexpr int SERVO_NEUTRAL_ANGLE = 90;       // Физическая нейтраль
                                
            //                    // Логический диапазон управления (относительно нейтрали)
            //                    constexpr int LOGICAL_MIN = -90;        // Макс. отклонение вниз/влево
            //                    constexpr int LOGICAL_MAX = +90;        // Макс. отклонение вверх/вправо
            //                    constexpr int LOGICAL_NEUTRAL = 0;      // Логическая нейтраль
