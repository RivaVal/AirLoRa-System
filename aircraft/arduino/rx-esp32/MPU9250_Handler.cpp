...
bool MPU9250Handler::begin(I2CMasterController& i2c_manager) {
    ESP_LOGI(TAG, "=== 🚀 ИНИЦИАЛИЗАЦИЯ MPU-9250 (9-AXIS) ===");
    _i2cManager = &i2c_manager;

    if (!_initMPU6500()) {
        ESP_LOGE(TAG, "❌ Ошибка инициализации MPU6500");
        return false;
    }
    if (!_enableI2CBypass()) {
        ESP_LOGE(TAG, "❌ Ошибка включения I2C Bypass для AK8963");
        return false;
    }
    if (!_initAK8963()) {
        ESP_LOGE(TAG, "❌ Ошибка инициализации магнитометра AK8963");
        return false;
    }

    _initialized = true;
    ESP_LOGI(TAG, "✅ MPU-9250 инициализирован. Калибровка гироскопа запущена...");
    return true;
}
...
bool MPU9250Handler::updateSensors() {
    ...
    uint32_t now = millis();
    int16_t rawTemp = 0;  // 🔧 Явная инициализация вместо int16_t rawTemp;
    float dt = (now - _lastUpdate) / 1000.0f;
    if (dt < 0.005f) return true; // Защита от слишком частого вызова (<5мс)
    _lastUpdate = now;
    ...
    if (!_readRawData(rawAccel, rawGyro, rawTemp, rawMag)) {
        ESP_LOGW(TAG, "⚠️ Сбой чтения данных с датчиков");
        return false;
    }

    // 🔑 Калибровка гироскопа (первые 200 сэмплов в покое)
    if (!_gyroCalibrated && _calibrationCount < 200) {
        ...
        if (_calibrationCount == 200) {
            ...
            _sensorData.status |= 0x03; // Биты 0 и 1: валидно + калибровано
            ESP_LOGI(TAG, "✅ Калибровка гироскопа завершена. Offset: X=%.1f Y=%.1f Z=%.1f",
                     _gyroOffsetX, _gyroOffsetY, _gyroOffsetZ);
        }
        return true; // Данные ещё не готовы для стабилизации
    }

    // 🔑 Масштабирование (±2g, ±250°/с)
    constexpr float ACCEL_SCALE = 9.80665f / 16384.0f;   // м/с²
    constexpr float GYRO_SCALE  = (250.0f / 32768.0f) * (M_PI / 180.0f); // рад/с
    ...
    // 🔑 Магнитометр с учётом чувствительности и смещения (мкТл)
...
bool MPU9250Handler::_initMPU6500() {
    ...
    if (!_i2cManager->writeRegister(MPU6500_ADDR, PWR_MGMT_1, &data, 1)) {
        ESP_LOGE(TAG, "❌ Не удалось выйти из сна MPU6500");
        return false;
    }
    delay(10);
    ...
    ESP_LOGI(TAG, "✅ MPU6500 инициализирован (WHO_AM_I=0x%02X)", whoami);
    return true;
}
...
bool MPU9250Handler::_enableI2CBypass() {
    ESP_LOGI(TAG, "⚙️  _enableI2CBypass: включение bypass...");
    ...
    ESP_LOGI(TAG, "✅ I2C Bypass включён, AK8963 обнаружен (0x%02X)", whoami);
    return true;
}
...
bool MPU9250Handler::_initAK8963() {
    ESP_LOGI(TAG, "⚙️  _initAK8963: настройка магнитометра...");
    
    // Сначала перевести в Power-Down режим
    uint8_t data = 0x00;
    if (!_i2cManager->writeRegister(AK8963_ADDR, AK8963_CNTL1, &data, 1)) {
        ESP_LOGE(TAG, "❌ Не удалось перевести AK8963 в Power-Down");
        return false;
    }
    delay(10);
    ...
    data = 0x0F;
    if (!_i2cManager->writeRegister(AK8963_ADDR, AK8963_CNTL1, &data, 1)) {
        ESP_LOGE(TAG, "❌ Не удалось войти в режим Fuse ROM");
        return false;
    }
    delay(10);
    ...
    ESP_LOGI(TAG, "   📊 ASA: [%d,%d,%d] → Sens: [%.3f,%.3f,%.3f] uT/LSB",
             asa[0], asa[1], asa[2],
...
bool MPU9250Handler::_readRawData(int16_t accel[3], int16_t gyro[3], int16_t temp, int16_t mag[3]) {
    ...
    if (!_i2cManager->readRegister(MPU6500_ADDR, 0x3B, buffer, 14)) {
        ESP_LOGW(TAG, "⚠️ Сбой чтения MPU6500 данных");
        return false;
    }
    ...
    if (!_i2cManager->readRegister(AK8963_ADDR, 0x02, magBuffer, 7)) {
        ESP_LOGW(TAG, "⚠️ Сбой чтения AK8963 данных");
    ...
    if ((magBuffer[0] & 0x01) == 0 || (magBuffer[0] & 0x08) != 0) {
        ESP_LOGW(TAG, "⚠️ AK8963: данные не готовы или ошибка");
        mag[0] = mag[1] = mag[2] = 0;
